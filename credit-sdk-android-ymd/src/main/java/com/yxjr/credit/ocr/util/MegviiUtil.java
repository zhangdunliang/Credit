...
public class MegviiUtil {
	...
	public static String getUUIDString(Context mContext) {
		...
		String uuid = YxStoreUtil.get(mContext, KEY_UUID);
		if (uuid != null)
			return uuid;
		uuid = YxAndroidUtil.getNativePhoneNumber(mContext);
		Log.w("ceshi", "uuid====" + uuid);
		if (uuid == null || uuid.trim().length() == 0) {
			uuid = getMacAddress(mContext);
			if (uuid == null || uuid.trim().length() == 0) {
				uuid = YxAndroidUtil.getDeviceId(mContext);
				if (uuid == null || uuid.trim().length() == 0) {
		...
		YxStoreUtil.save(mContext, KEY_UUID, uuid);
		return uuid;
	}
