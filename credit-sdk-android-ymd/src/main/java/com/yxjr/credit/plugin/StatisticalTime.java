...
public class StatisticalTime {
	...
	public void init(Context context) {
		mContext = context;
		//取用户的基本信息
		custName = YxStoreUtil.get(mContext, SpConstant.PARTNER_REAL_NAME);
		custMobile = YxStoreUtil.get(mContext, SpConstant.PARTNER_PHONENUMBER);
		custCert = YxStoreUtil.get(mContext, SpConstant.PARTNER_ID_CARD_NUM);
		...
		phoneOsType = "android";
		systemPageVersion = YxCommonConstant.SDK_VERSION;
	}
	...
	private StatisticalTime(Builder builder) {
	...
	private void toSend() {
		...
		try {
			//创建连接 
			URL url = new URL(HttpConstant.PULL_TIME_URL);
			...
			byte[] outputByte = new byte[HttpConstant.BUFFER_SIZE];
			int count = 0;
			...
			while ((count = httpInputStream.read(outputByte, 0, HttpConstant.BUFFER_SIZE)) != -1) {
	...
	public static class Builder {
		...
		public StatisticalTime build() {
			return new StatisticalTime(this);
		}
