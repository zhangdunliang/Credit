...
public final class ResContainer {
	...
	public static int getResourceId(Context paramContext, String paramString1, String paramString2) {
		...
		if (i <= 0) {
			throw new RuntimeException("获取资源ID失败:(packageName=" + mPackageName + " type=" + paramString1 + " name=" + paramString2);
		}
		return i;
	}
	...
	// }
	// Set localSet = this.mResources.keySet();
	// for (String str : localSet) {
	// SocializeResource localSocializeResource = (SocializeResource)
	// this.mResources.get(str);
	// localSocializeResource.mId = getResourceId(this.context,
	// localSocializeResource.mType, localSocializeResource.mName);
