	...
	public void uploadP1() {
		...
		new RequestEngine(mActivity).upload(mAppNo, files, new UploadCallBack() {
			...
			public void onSucces(String result) {
				...
				showError(IDCardSide.IDCARD_SIDE_FRONT, "身份证正面照片上传成功", mAppNo);
			...
			public void onFailure(String errorCode, String errorMsg) {
				showError(IDCardSide.IDCARD_SIDE_FRONT, "身份证正面照片上传失败" + errorMsg, mAppNo);
...
//				}
	...
	public void uploadP2() {
		...
		new RequestEngine(mActivity).upload(mAppNo, files, new UploadCallBack() {
			@Override
			public void onSucces(String result) {
				showError(IDCardSide.IDCARD_SIDE_BACK, "身份证反面照片上传成功", mAppNo);
				send(P2, result, null);
				super.onSucces(result);
			}
			...
			@Override
			public void onFailure(String errorCode, String errorMsg) {
				showError(IDCardSide.IDCARD_SIDE_BACK, "身份证反面照片上传失败" + errorMsg, mAppNo);
	...
	public void send(final String picType, String certFilePath, String certPortraitFilePath) {
		final IDCardSide side = picType.equals(P1) ? IDCardSide.IDCARD_SIDE_FRONT : IDCardSide.IDCARD_SIDE_BACK;
				...
				if (responseCode != null && responseCode.equals(HttpConstant.Response.SUCCEED)) {
					...
					if (picType != null) {
						if (picType.equals(P1)) {
							...
							mActivity.finish();
							if (mScanType == 0) {
								ScanIDCardActivity.startMe(mActivity, IDCardSide.IDCARD_SIDE_BACK);
							}
	...
	public void showError(IDCardSide side, String errorStr, String sessionId) {
		new StatisticalTime.Builder().addContext(mActivity).addErrorInfo(errorStr).addOperPageName(side == IDCardSide.IDCARD_SIDE_FRONT ? "Face++身份证识别-正面" : "Face++身份证识别-反面").addOperElementType("ocr")
