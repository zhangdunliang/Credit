...
public final class YxCredit {
    ...
    public void init(Context context) {
        ...
        if (!BuildConfig.YMJ_RELEASE) {
            ToastUtil.showToast(context, BuildConfig.FLAVOR + " [" + BuildConfig.YMJ_SDK_VERSION + "]");
        }
    }
    ...
    private void execute(Context context, Bundle bundle, String type) {
        if (initState == null) {
            ...
            if (bundle == null) {
                ...
                // else if (!YxCommonUtil.isNotBlank(packageName)) {
                // mCallBcak.onFailure(errorMsg.CODE_20006,
                // errorMsg.getMessageInfo(errorMsg.CODE_20006));
                // } else if (!YxCommonUtil.isNotBlank(className)) {
                // mCallBcak.onFailure(errorMsg.CODE_20007,
                // errorMsg.getMessageInfo(errorMsg.CODE_20007));
                // }
                else {
                    if (mCallBcak != null) {
