...
public class RequestFactory {
	...
	public HashMap<String, String> getHttpHead(String serviceId) {
		...
		httpHead.put("partnerLoginId", Md5.encrypt(partnerLoginId));
	...
	public JSONObject getBodyHead(Context context) {
		...
		try {
			...
			header.put("partnerLoginId", Md5.encrypt(partnerLoginId));// 身份证
			header.put("appFlag", "OSB");//标识为益秒借，不传或者传值不正确，默认为“益秒到”（OST），因此益秒到不需要传也可兼容

			JSONObject product = new JSONObject();
			product.put("productId", "6003");// 产品ID
			product.put("productIdChild", "600301");// 产品子ID
	...
	public void parseServiceBody(Context mContext, JSONObject serviceBody) throws JSONException {
		...
		/** 系统最低版本支持 */
		String SUPPORT_SDK_VERSION_YMJ = "SUPPORT_SDK_VERSION_YMJ";
		/** 通讯录每次发送量 */
		String CSQ = "CSQ";
		/** 通讯录是否发送 */
		String C = "C";
		...
		String SUPPORT_SDK_VERSION_YMJ_P = null;
		String CSQ_P = null;
		String C_P = null;
		...
		if (!serviceBody.isNull(SUPPORT_SDK_VERSION_YMJ)) {
			SUPPORT_SDK_VERSION_YMJ_P = serviceBody.getString(SUPPORT_SDK_VERSION_YMJ);
		}
		...
		if (YxCommonUtil.isNotBlank(SUPPORT_SDK_VERSION_YMJ_P)) {
			YxStoreUtil.save(mContext, SpConstant.SUPPORT_SDK_VERSION_YMJ, SUPPORT_SDK_VERSION_YMJ_P);
		} else {
			YxStoreUtil.save(mContext, SpConstant.SUPPORT_SDK_VERSION_YMJ, "1.0.1");
		}
		...
		if (YxCommonUtil.isNotBlank(PSQ_P)) {
			...
			YxStoreUtil.save(mContext, SpConstant.PSQ, "20");
		}
